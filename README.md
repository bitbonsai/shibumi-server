...
> Experimental: release `0.2.3` is being dogfooded.
...
```bash
bunx shibumi-server@0.2.3 init
shis add example.com \
...
`init` stores the release under `~/.local/share/shibumi-server/releases/0.2.3`, updates the local `current` symlink and launcher, and prepares the config, secrets, and systemd service. Re-running it preserves machine config and secrets. `add` validates the complete app config. To run app-owned tests before startup, append an optional argument array such as `-- bun test`; it is never interpreted as a shell string.
...
`shis uninstall --purge` also deletes config and webhook secrets after confirmation. Automation must make that irreversible choice explicit with `--purge --yes`. Neither mode removes app checkouts, containers, Caddy routes, or GitHub settings.
