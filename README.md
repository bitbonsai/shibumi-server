...
> Experimental: release `0.2.14` is being dogfooded.
...
```bash
bunx shibumi-server@0.2.14 init
shis add example.com \
...
`init` stores the release under `~/.local/share/shibumi-server/releases/0.2.14`, updates the local `current` symlink and launcher, and prepares the config, secrets, and systemd service. Re-running it preserves machine config and secrets. `add` validates the complete app config. To run app-owned tests before startup, append an optional argument array such as `-- bun test`; it is never interpreted as a shell string.
