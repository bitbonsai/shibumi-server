...
- Installed releases expose compatible `shis` and `shibumi-server` launchers. Human-facing help and interactive flows brand themselves exactly `渋み  shis (shibumi-server)` and prefer concise `shis` commands. Human summaries use `terminal-ui.ts` stage labels matching website terminal language: quiet persimmon neutral labels, green confirmation, persimmon action, and exactly one solid green final outcome. Respect `NO_COLOR`, non-TTY output, and `TERM=dumb`; machine-readable JSON commands stay free of decorative output.
...
- Release version digits must add up to one of Mauricio's preferred totals: `1`, `2`, `3`, `5`, `7`, or `9`. Enforce this in package tests; `0.1.40` is valid because its digits total `5`.
